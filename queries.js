...
const getStyles = async (id) => {
  ...
  for(var i = 0; i < data.length; i++) {
    data[i].photos = await getPhotos(data[i].id).catch((err) => {
      console.log(err);
    });
    data[i].skus = await getSkus(data[i].id).catch((err) => {
      console.log(err);
    });
  }
...
const getPhotos = async (id) => {
  var data = await db.query(`SELECT base_url, thumbnail_url FROM photos WHERE photos.style = ${id}`).catch((err) => {
    console.log(err);
  })
  return data;
}
...
const getSkus = async (id) => {
  var data = await db.query(`SELECT id, quantity, size FROM skus WHERE skus.style = ${id}`).catch((err) => {
    console.log(err);
  })
  return data;
}
